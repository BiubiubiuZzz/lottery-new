...
//抽奖码使用
func (this *LotteryController) GetQrcodeUsed() {
	Logined := this.GetSession("login")
	if Logined != true {
		this.Redirect("/login", 302)
		return
	...
	uid,_ :=this.GetSession("uid").(int)
	deliverID := GetDeliverIDByUid(uid)
	list := QRused(deliverID)
	this.Data["qruse"] =list
	quantity :=QRusedQ(deliverID)
	this.Data["qrquantity"] = quantity

	this.Data["queryValue"] = ""
	this.TplName = "qr_use.html"
}

//抽奖码未使用
func (this *LotteryController) GetQrcodeNotUsed() {
	Logined := this.GetSession("login")
	if Logined != true {
		this.Redirect("/login", 302)
		return
	...
	uid,_ :=this.GetSession("uid").(int)
	deliverID := GetDeliverIDByUid(uid)
	list := QRNotUsed(deliverID)
	this.Data["qrnotuse"] =list
	quantity :=QRNotUsedQ(deliverID)
	this.Data["qrquantity"] = quantity //抽奖码未使用数量
	this.Data["queryValue"] = ""
	this.TplName = "qr_notuse.html"
}
...
func (this *LotteryController) GetAddress() {
	...
	this.Data["lists"] = list
	quantity := GetAddressQ(deliverID)
